...
function initJsonFileTable(msg){
	...
	for(var i=0;i<msg.result.biz.length;i++){
		var html="<tr><td>"+msg.result.biz[i].bizName+"</td><td><a target='_blank' href='<%=path%>"+msg.result.biz[i].bizPath+"'>"+msg.result.biz[i].bizName+"</a></td><td>"+msg.result.biz[i].lastModify+"</td><td><a href='javascript:editDevBiz(\""+msg.result.biz[i].bizName+"\")'>查看</a></td></tr>";
