...
function parseJson2DS(msg){
	 ...
	 for(var i in ds){
    	 ...
    	 if(beanClass==c3p0){
        	 ...
        	 delete props["user"];
        	 obj.find("input:text[name='password']").val(props["password"]);
...
function paresJson2Biz(msg){
	var obj=$("#biz_m").clone();
	if(msg!=null){
		for(var i in msg){
			var p=$('<tr><td width="20%" style="line-height:35px">'+i+'</td><td width="75%"><input type="text" class="form-control" name="'+i+'" value=""></td><td width="5%"  style="text-align:right"><span class="glyphicon glyphicon-minus"></span></td></tr>');
			p.find("input.form-control").val(msg[i]);
	...
	//增加属性
	 obj.find("span.glyphicon-plus").click(function(){
		...
		if($(this).attr("class").indexOf("dss")!=-1){
		  ...
		  p=$('<tr><td width="20%" style="line-height:35px"><input type="text" class="form-control bizPropsAddKey"></td><td width="75%"><input type="text" class="form-control bizPropsAddValue"></td><td width="5%"  style="text-align:right"><span class="glyphicon glyphicon-minus"></span></td></tr>');
	...
	if(checkNull()){
		$("div.bizInfo:visible").each(function(){
			...
			obj.find("input:text[class*='bizPropsAddKey']").each(function(){
				jsons[$(this).val()]=$(this).parent().siblings("td").find("input:text[class*='bizPropsAddValue']").val();
			});
		});
		return true;
	}else{
