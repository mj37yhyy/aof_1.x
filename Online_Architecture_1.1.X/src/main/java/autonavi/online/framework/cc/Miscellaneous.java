...
public class Miscellaneous {
	...
	static public int aof_messageListener_prot = -1;// 消息监听时的端口号
	...
	static private int nodeIndex=0;

	public static int getNodeIndex() {
		return nodeIndex;
	}

	public static boolean setNodeIndex(int nodeIndex) {
		if (nodeIndex >= minNodeKey && nodeIndex <= maxNodeKey){
			Miscellaneous.nodeIndex = nodeIndex;
			return true;
		}
		return false;
	...
	public int getAof_messageListener_prot() {
		return aof_messageListener_prot;
	}

	public void setAof_messageListener_prot(int aof_messageListener_prot) {
		Miscellaneous.aof_messageListener_prot = aof_messageListener_prot;
	}
