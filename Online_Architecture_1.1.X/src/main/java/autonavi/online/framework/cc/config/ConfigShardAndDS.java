...
import autonavi.online.framework.jdbc.dao.HbmHelper;
...
public class ConfigShardAndDS {
	 ...
	 * @throws Exception
	 */
	public void initShard(CcBaseEntity ccBaseEntity,
...
//					dataSourceRouteBeanDefinition);
	...
	private Map<Integer, Object> initDS(CcBaseEntity ccBaseEntity)
		...
		for (String key : realDataSources.keySet()) {
			...
			realMap.put(key, (DataSource) CcConfigUtils.getInstance()
					.generateObject(ds.getBeanClass(), propertyMap));
		}
		// 代理AOF数据源初始化
	...
	private RootBeanDefinition initShardDS(CcBaseEntity ccBaseEntity,
			Map<Integer, Object> map) throws ExistingDataSourceKeyException,
	...
	private void initHibernateUtils(final DataSource indexDS) {
		...
		CcConfig.scanPipeline.add(new ScanAllClassHandle() {
			@Override
		 ...
		 * 将要扫描完处理的东西放入后置处理器
		 */
		CcConfig.scanPipelineAfters.add(new ScanPipelineAfter() {
			@Override
