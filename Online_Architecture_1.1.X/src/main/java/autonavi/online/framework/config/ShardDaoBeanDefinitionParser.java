...
public class ShardDaoBeanDefinitionParser implements BeanDefinitionParser {
	...
	public BeanDefinition parse(Element element, ParserContext parserContext) {
		...
		// 将Map置入ManagedMap中去
		ManagedMap<Class<?>, Map<String, SqlSource>> mm = new ManagedMap<Class<?>, Map<String, SqlSource>>() {
			{
				putAll(classesHolder.get());// 从本地线程中得到组合好的map
			}
		};
		classesHolder = null;// 析构classesHolder

		// 生成SqlSession的Bean
		RootBeanDefinition SqlSessionBeanDefinition = new RootBeanDefinition(
				SqlSession.class);
		SqlSessionBeanDefinition.getPropertyValues().add("classes", mm);
