...
import autonavi.online.framework.jdbc.transactional.ConnectionTransactionManage;
import autonavi.online.framework.jdbc.transactional.holder.TimeOutHolder;
import autonavi.online.framework.jdbc.transactional.holder.TransactionAttributeHolder;
import autonavi.online.framework.jdbc.transactional.holder.TransactionHolder;
import autonavi.online.framework.jdbc.transactional.holder.TransactionManagerHolder;
...
public class DaoSupport {
		...
		try {
			if (tableOperation == TableOperation.Ddl) {
				...
				if (checkIsAofIndex(daoEntity)) {
					...
					sqlHelper.executeSql(daoEntity.getIndexName(),
							daoEntity.getIndexColumn(), indexColumnValue,
							daoEntity.getSingleDataSourceKey(),
							daoEntity.getAuthor(), daoEntity.getSql(),
							daoEntity.getParameterMap(),
							daoEntity.getShardingHandle(),daoEntity.isOpenTx());
				}
				this.commit();
			} else {
	...
	private HbmHelper hbmHelper = null;
