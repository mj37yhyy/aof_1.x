...
import autonavi.online.framework.jdbc.dao.paging.PagingFactory;
import autonavi.online.framework.jdbc.dao.paging.entity.Count;
import autonavi.online.framework.jdbc.datasource.CustomerContextHolder;
import autonavi.online.framework.jdbc.datasource.DataSourceRoute;
import autonavi.online.framework.jdbc.datasource.NamedParameterStatement;
import autonavi.online.framework.jdbc.datasource.RegularExpressions;
import autonavi.online.framework.jdbc.datasource.ResultSetCallback;
import autonavi.online.framework.jdbc.datasource.RouteResult;
...
public class SqlHelper {
	...
	protected Object select(String author, String indexName,
			...
			int endOrRowSize, boolean isQueryCount, boolean isReturnOne,
		...
		try {
			...
			if (callback != null && !callback.getClass().isInterface()) {// 如果存在回调函数，则进行调用
				...
				h = this.getResultSetHandler(resultType, isReturnOne);// 返回ResultSetHandler的实例
	...
	protected List<Object> selectBatch(String author, String indexName,
			...
			boolean queryCount, boolean returnOne, Class<?> resultType,
		...
		for (Iterator<Integer> i = sqls.keySet().iterator(); i.hasNext();) {
			...
			try {
				...
				if (callback != null && !callback.getClass().isInterface()) {// 如果存在回调函数，则进行调用
					...
					h = this.getResultSetHandler(resultType, returnOne);// 返回ResultSetHandler的实例
	...
	private ResultSetHandler getResultSetHandler(Class<?> resultType,
			boolean isReturnOne) {
		if (resultType == null)// 默认是mapList
			return new MapListHandler(new BasicRowProcessorPatch());
		if (resultType.isArray()) {// 如果数组
			if (isReturnOne)
				return new ArrayHandler();
			else
				return new ArrayListHandler();
		} else if (resultType.isPrimitive() || resultType.equals(Integer.class)
				...
				|| resultType.equals(Byte.class)
				|| resultType.equals(Boolean.class)
				|| resultType.equals(String.class)) {// 如果是基本类型或基本类型的对像或String
			if (isReturnOne)
				return new ScalarHandler();
			else
				return new ColumnListHandler();
		} else if (Map.class.isAssignableFrom(resultType)) {// 如果是Map
			if (isReturnOne)
	...
	private DataSourceRoute dataSourceRoute = null;
