package autonavi.online.framework.sharding.dao.xml.builder;
...
import autonavi.online.framework.exception.UsePrimitiveException;
...
import autonavi.online.framework.sharding.dao.xml.builder.support.mapping.BoundSql;
import autonavi.online.framework.sharding.dao.xml.builder.support.mapping.SqlSource;
import autonavi.online.framework.sharding.dao.xml.builder.support.session.SqlSession;
	...
	public Class<?> createImplClassFromInterface(Class<?> interfaceClass)
		...
		try {
			if (interfaceClass != null) {
				...
				String className = interfaceClass.getName() + "$JavassistImpl";// 子类名
				...
				if (methodMap != null) {// 如果解析没有问题，得到了map
					...
					classInfo
							.setImportPackages(Map.class.getName(),
									HashMap.class.getName(),
									DaoEntity.class.getName(),
									TableOperation.class.getName(),
									PropertyUtils.class.getName(),
									DaoSupport.class.getName(),
									SqlSession.class.getName(),
									SqlSource.class.getName(),
									BoundSql.class.getName());// 引入的类
					classInfo.setFields(
							"private DaoSupport daoSupport = null;",
							"private SqlSession sqlSession = null;");
					String[] methods = new String[methodMap.size() + 2];
					methods[0] = "public void setDaoSupport(DaoSupport daoSupport) {this.daoSupport = daoSupport;}";
