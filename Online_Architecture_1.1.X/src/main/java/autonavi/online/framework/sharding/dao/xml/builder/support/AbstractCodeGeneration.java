...
import autonavi.online.framework.annotation.sharding.ResultSetCallBackHandler;
import autonavi.online.framework.annotation.sharding.SqlParameter;
import autonavi.online.framework.config.ShardDaoBeanDefinitionParser;
import autonavi.online.framework.exception.MustContainAnnotationException;
import autonavi.online.framework.jdbc.datasource.ResultSetCallback;
import autonavi.online.framework.sharding.dao.xml.builder.support.mapping.SqlSource;
import autonavi.online.framework.sharding.dao.xml.builder.support.xmltags.XMLScriptBuilder;
...
public abstract class AbstractCodeGeneration {
	...
	protected Map<String, Object> getParamMap(Class<?> interfaceClass, Node node)
		...
		for (Method targetMethod : interfaceClass.getMethods()) {
			if (targetMethod.getName().equals(id)) {// 如果是当前方法
				...
				// 是否返回单对象
				boolean returnOne = true;
				if (List.class.isAssignableFrom(targetMethod.getReturnType())
						|| targetMethod.getReturnType().isArray()) {// 如果是list或数组
					returnOne = false;
				}
				paramMap.put("returnOne", returnOne);// 是否一条返回
	...
	protected String parseSql(Class<?> interfaceClass, Node node)
		...
		Map<Class<?>, Map<String, SqlSource>> classes = ShardDaoBeanDefinitionParser.classesHolder
				.get();
		...
		ShardDaoBeanDefinitionParser.classesHolder.set(classes);
